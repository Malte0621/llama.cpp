...
static std::vector<int32_t> read_i32(const char * path) {
    ...
    if (sz < 0 || sz % (long) sizeof(int32_t) != 0) {
        fprintf(stderr, "invalid int32 file size for %s\n", path);
        fclose(f);
        exit(1);
    }
    fseek(f, 0, SEEK_SET);
    std::vector<int32_t> v((size_t) sz / sizeof(int32_t));
    if (fread(v.data(), sizeof(int32_t), v.size(), f) != v.size()) {
...
static std::vector<float> read_f32(const char * path) {
    ...
    if (sz < 0 || sz % (long) sizeof(float) != 0) {
        fprintf(stderr, "invalid float32 file size for %s\n", path);
        fclose(f);
        exit(1);
    }
    fseek(f, 0, SEEK_SET);
    std::vector<float> v((size_t) sz / sizeof(float));
...
int main(int argc, char ** argv) {
    ...
    if (prompt_ids.size() > INT_MAX || canvas_ids.empty() || canvas_ids.size() > INT_MAX ||
        prompt_ids.size() + canvas_ids.size() > INT_MAX) {
        fprintf(stderr, "prompt/canvas token counts are out of range\n");
        return 1;
    }
    ...
    if (!llama_model_is_diffusion(model)) {
        fprintf(stderr, "model is not a diffusion model\n");
        return 1;
    }
