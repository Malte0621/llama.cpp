...
static std::vector<int32_t> read_i32_file(const std::string & path) {
    ...
    fseek(f, 0, SEEK_END);
    long sz = ftell(f);
    if (sz < 0 || sz % (long) sizeof(int32_t) != 0) { fclose(f); return {}; }
    fseek(f, 0, SEEK_SET);
    std::vector<int32_t> v((size_t) sz / sizeof(int32_t));
    if (fread(v.data(), sizeof(int32_t), v.size(), f) != v.size()) { fclose(f); return {}; }
    fclose(f);
    return v;
}
...
int main(int argc, char ** argv) {
    ...
    if (MAXTOK <= 0) {
        fprintf(stderr, "MAXTOK must be positive\n");
        return 1;
    }
    ...
    if (!llama_model_is_diffusion(model) || n_vocab <= 0) {
        fprintf(stderr, "model is not a valid diffusion model\n");
        return 1;
    }
    char canvas_meta[32] = {};
    ...
    char * canvas_end = nullptr;
    const long model_canvas = strtol(canvas_meta, &canvas_end, 10);
    if (canvas_end == canvas_meta || *canvas_end != '\0' || model_canvas <= 0 ||
        model_canvas > INT_MAX || model_canvas > MAXTOK) {
        fprintf(stderr, "diffusion.canvas_length is invalid or exceeds MAXTOK\n");
        return 1;
    }
    ...
    cparams.n_outputs_max = (uint32_t) model_canvas;
    ...
    while (fgets(line, sizeof(line), stdin)) {
        ...
        if (P < 0 || C <= 0 || C != (int) model_canvas || n64 <= 0 || n64 > MAXTOK) {
            printf("ERR bounds %d %d\n", P, C); fflush(stdout); continue;
        }
        const int N = (int) n64;
        ...
        float temp   = 1.0f;
        size_t hdr   = 2;
        if (req.size() == (size_t) N + 4) {
            hdr = 4;
            use_sc = req[2];
            memcpy(&temp, &req[3], sizeof(float));
        } else if (req.size() != (size_t) N + 2) {
            printf("ERR badsize %d %zu\n", N, req.size()); fflush(stdout); continue;
        }
        if ((use_sc != 0 && use_sc != 1) || !std::isfinite(temp) || temp <= 0.0f) {
            printf("ERR badsc\n"); fflush(stdout); continue;
        }

        const size_t sc_size = (size_t) C * (size_t) n_vocab;
        if (sc_cache.size() != sc_size) {
            sc_cache.assign(sc_size, 0.0f);
        }
