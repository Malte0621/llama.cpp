...
static bool vis_step_callback(int32_t step, int32_t total_steps, const llama_token * tokens,
    ...
    const std::string json_text = nlohmann::json(text).dump(-1, ' ', false, nlohmann::json::error_handler_t::replace);
    fprintf(d->out, "F %d %d %d %s\n", d->block, step, total_steps, json_text.c_str());
    fflush(d->out);
...
int main(int argc, char ** argv) {
    ...
    if (n_vocab <= 0) { fprintf(stderr, "model has an invalid vocabulary\n"); return 1; }
    ...
    int64_t canvas_length = 0;
    {
        ...
        if (llama_model_meta_val_str(model, "diffusion.canvas_length", canvas_str, sizeof(canvas_str)) >= 0) {
            ...
            if (end == canvas_str || *end != '\0') {
                canvas_length = 0;
            }
        }
    }
    if (canvas_length <= 0 || canvas_length > INT_MAX / 4) {
        fprintf(stderr, "model has an invalid diffusion.canvas_length\n");
        return 1;
    }
    ...
    const bool one_sampling_device = sampling_devs == 1 && mparams.n_gpu_layers != 0;
    const bool pkv_supported = gpu_devs <= 1;
    ...
    base.kv_cache          = pkv_supported;
    base.gpu_sampling      = one_sampling_device;
    base.gpu_sample_reduce = one_sampling_device;
    ...
    while (fgets(line, sizeof(line), stdin)) {
        ...
        if (prefix.size() > INT_MAX || n_blocks <= 0 ||
            (int64_t) n_blocks * canvas_length > INT_MAX) {
            printf("ERR bounds\n"); fflush(stdout); continue;
        }
        ...
        for (int b = 0; b < n_blocks; b++) {
            if (prefix.size() > INT_MAX) {
                printf("ERR toolong %zu %d\n", prefix.size(), MAXTOK); fflush(stdout);
                break;
            }
            const int32_t prefix_len = (int32_t) prefix.size();
            const int64_t needed = (int64_t) prefix_len + canvas_length;
            if (needed > MAXTOK) {
                printf("ERR toolong %lld %d\n", (long long) needed, MAXTOK); fflush(stdout);
                break;
            }
            ...
            eb.seed                    = (int32_t) ((uint32_t) seed + (uint32_t) b);
            ...
            const std::string json_answer = nlohmann::json(answer_text).dump(-1, ' ', false, nlohmann::json::error_handler_t::replace);
            printf("C %d %s\n", b, json_answer.c_str()); fflush(stdout);
