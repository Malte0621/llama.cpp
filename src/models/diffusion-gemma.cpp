...
llama_model_diffusion_gemma::graph::graph(const llama_model & model, const llm_graph_params & params) :
    ...
    auto dg_canvas_embed = [&](ggml_tensor * canvas) -> ggml_tensor * {
        ...
        if (dmodel.sc_enabled) {
            ...
            if (dmodel.sc_device_resident) {
                ...
                sc_logits = ggml_view_2d(ctx0, dmodel.sc_dev, n_vocab, Cc, dmodel.sc_dev->nb[1], 0);
            } else {
            ...
            ggml_tensor * g = ggml_gelu(ctx0, build_lora_mm(model.sc_gate, normed)); // [n_ff, Cc]
            ggml_tensor * u = build_lora_mm(model.sc_up, normed);                    // [n_ff, Cc]
            ggml_tensor * sc_sig = build_lora_mm(model.sc_down, ggml_mul(ctx0, g, u)); // [n_embd, Cc]
    ...
    if (dmodel.sc_enabled && dmodel.sc_device_resident && C > 0) {
        ...
        ggml_tensor * sc_dst = ggml_view_2d(ctx0, dmodel.sc_dev, n_vocab, C,
                                            dmodel.sc_dev->nb[1], 0);
        ggml_tensor * sc_cpy = ggml_cpy(ctx0, canvas_logits, sc_dst);
...
// Stage-1 device sampling entry. Fetch the sampler from the backend that owns sc_dev, rather than
// probing global registries and risking dispatch through a backend that cannot access this tensor. Returns
// false when that backend does not provide the CUDA-family sampler so the caller can use the host path.
...
bool llama_diffusion_device_sample(const struct llama_model * model, const float * u, int * argmax,
    ...
    if (!dm || dm->sc_dev == nullptr || dm->sc_dev->buffer == nullptr ||
        !u || !argmax || !entropy || !sampled || n_tokens <= 0 || n_tokens > dm->sc_dev_C) {
        return false;
    }

    ggml_backend_buffer_type_t buft = ggml_backend_buffer_get_type(dm->sc_dev->buffer);
    ...
    const auto fn = (dg_cuda_sample_fn) ggml_backend_reg_get_proc_address(
        reg, "ggml_backend_cuda_diffusion_sample");
    if (!fn) {
        return false;
    }
...
void llama_diffusion_set_phase(struct llama_model * model, int phase, int32_t P, int32_t off) {
    ...
    if (phase < llama_model_diffusion_gemma::PKV_UNIFIED ||
        phase > llama_model_diffusion_gemma::PKV_DECODE || P < 0 || off < 0 ||
        (phase == llama_model_diffusion_gemma::PKV_PREFILL && off > P)) {
        dm->pkv_phase       = llama_model_diffusion_gemma::PKV_UNIFIED;
        dm->pkv_P           = 0;
        dm->pkv_prefill_off = 0;
        return;
    }
