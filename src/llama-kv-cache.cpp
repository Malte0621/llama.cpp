...
llama_kv_cache::llama_kv_cache(
    ...
    auto ctx_for_buft = [&](ggml_backend_buffer_type_t buft) -> ggml_context * {
        ...
        if (it == ctx_map.end()) {
            ggml_init_params params = {
                // Reserve K, V, optional K_idx, stream views, and one shared InnerQ tensor.
                /*.mem_size   =*/ size_t((3u*(1 + n_stream)*n_layer + 1)*ggml_tensor_overhead()),
                /*.mem_buffer =*/ NULL,
                /*.no_alloc   =*/ true,
            };
    ...
    const int adaptive_mode = [&]() {
        ...
        if (type_v == GGML_TYPE_TURBO2_0 && hparams.n_layer() >= 8) {
            LLAMA_LOG_INFO("%s: Boundary V auto-enabled for turbo2-V (opt-out: TURBO_LAYER_ADAPTIVE=0)\n", __func__);
            return 7;
        }
        return 0;
    }();
    ...
    if (other) {
        turbo_innerq_scale_inv = other->get_turbo_innerq_scale_inv();
    }
    ...
    for (uint32_t il = 0; il < n_layer; il++) {
        ...
        if (turbo_innerq_scale_inv == nullptr && (k_is_turbo || v_is_turbo)) {
    ...
    for (auto & [buft, ctx] : ctx_map) {
        ...
        if (!hparams.no_alloc && turbo_innerq_scale_inv != nullptr && turbo_innerq_scale_inv->buffer == buf) {
    ...
    if (other) {
        ...
        const bool attn_rot_disable = LLAMA_ATTN_ROT_DISABLE && atoi(LLAMA_ATTN_ROT_DISABLE) != 0;
        if (attn_rot_disable) {
        ...
        const char * rot_k_override = getenv("LLAMA_ATTN_ROT_K_OVERRIDE");
        ...
        const char * rot_v_override = getenv("LLAMA_ATTN_ROT_V_OVERRIDE");
...
void llama_kv_cache::clear(bool data) {
    ...
    if (data) {
        for (auto & [_, buf] : ctxs_bufs) {
            ...
            if (turbo_innerq_scale_inv != nullptr && turbo_innerq_scale_inv->buffer == buf.get()) {
                float ones[INNERQ_MAX_CHANNELS];
                for (int i = 0; i < INNERQ_MAX_CHANNELS; ++i) {
                    ones[i] = 1.0f;
                }
...
ggml_tensor * llama_kv_cache::get_k(ggml_context * ctx, int32_t il, uint32_t n_kv, const slot_info & sinfo, ggml_tensor * source) const {
    const int32_t ikv = map_layer_ids.at(il);

    auto * storage = layers[ikv].k;
    auto * k = source ? source : storage;
    GGML_ASSERT(k->type == storage->type && ggml_nelements(k) == ggml_nelements(storage));

    const uint64_t kv_size      = get_size();
    const uint64_t n_embd_k_gqa = storage->ne[0];
...
ggml_tensor * llama_kv_cache::get_v(ggml_context * ctx, int32_t il, uint32_t n_kv, const slot_info & sinfo, ggml_tensor * source) const {
    const int32_t ikv = map_layer_ids.at(il);

    auto * storage = layers[ikv].v;
    auto * v = source ? source : storage;
    GGML_ASSERT(v->type == storage->type && ggml_nelements(v) == ggml_nelements(storage));

    const uint64_t kv_size      = get_size();
    const uint64_t n_embd_v_gqa = storage->ne[0];
...
ggml_tensor * llama_kv_cache_context::get_k(ggml_context * ctx, int32_t il, ggml_tensor * source) const {
    return kv->get_k(ctx, il, n_kv, sinfos[i_cur], source);
}

ggml_tensor * llama_kv_cache_context::get_v(ggml_context * ctx, int32_t il, ggml_tensor * source) const {
    return kv->get_v(ctx, il, n_kv, sinfos[i_cur], source);
}
