...
static vk_fa_tuning_params get_fa_tuning_params(const vk_device& device, uint32_t hsk, uint32_t hsv, uint32_t n_rows, uint32_t n_kv, ggml_type k_type, ggml_type v_type, bool f32acc) {
    ...
    if (path == FA_COOPMAT2 && turbo_kv) {
        // TurboQuant dequantization is implemented by the scalar and coopmat1 shaders.
...
static uint32_t get_subgroup_size(const std::string &pipeline_name, const vk_device_architecture &arch) {
    // TQ4_1S has a fixed 32-thread shared-memory kernel.
    if (pipeline_name.find("mul_mat_vec_tq4_1s") != std::string::npos) {
        return 0;
    }
...
#undef CREATE_MM
    ...
    static constexpr uint32_t tq4_1s_wg_size = 32;
    static constexpr shader_reduction_mode tq4_1s_reduc = SHADER_REDUCTION_MODE_SHMEM;
...
#define OCP_DMMV_DATA(NAME, REDUC) NAME ## _data[REDUC]
    ...
    for (uint32_t w = 0; w < DMMV_WG_SIZE_COUNT; ++w) {
        ...
        for (uint32_t i = 0; i < mul_mat_vec_max_cols; ++i) {
            ...
            ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f32_f32[w][GGML_TYPE_TQ4_1S][i], "mul_mat_vec_tq4_1s_f32_f32", arr_dmmv_tq4_1s_f32_f32_len[tq4_1s_reduc], arr_dmmv_tq4_1s_f32_f32_data[tq4_1s_reduc], "main", mul_mat_vec_num_bindings, sizeof(vk_mat_vec_push_constants), {8, 1, 1}, {tq4_1s_wg_size, 8, i+1}, 1, true);
...
#undef OCP_DMMV_LEN
    ...
    ggml_vk_create_pipeline(device, device->pipeline_get_rows[GGML_TYPE_TQ4_1S], "get_rows_tq4_1s", get_rows_tq4_1s_len, get_rows_tq4_1s_data, "main", 3, sizeof(vk_op_binary_push_constants), {256 * 32, 1, 1}, {}, 1);
    ...
    ggml_vk_create_pipeline(device, device->pipeline_get_rows_f32[GGML_TYPE_TQ4_1S], "get_rows_tq4_1s_f32", get_rows_tq4_1s_f32_len, get_rows_tq4_1s_f32_data, "main", 3, sizeof(vk_op_binary_push_constants), {256 * 32, 1, 1}, {}, 1);
...
#define SET_ROWS(src_idx, src, itype) \
        ...
        ggml_vk_create_pipeline(device, device->pipeline_set_rows ## itype [src_idx][GGML_TYPE_TQ4_1S], "set_rows_" #src "_tq4_1s" #itype, set_rows_ ## src ## _tq4_1s ## itype ## _len, set_rows_ ## src ## _tq4_1s ## itype ## _data, "main", 3, sizeof(vk_op_binary_push_constants), {1, 1, 1}, {1}, 1, true, false, 32u);
...
static vk_pipeline ggml_vk_get_dequantize_mul_mat_vec(ggml_backend_vk_context * ctx, ggml_type a_type, ggml_type b_type, uint32_t num_cols, uint32_t m, uint32_t k) {
    ...
    if (a_type != GGML_TYPE_TQ4_1S && ((ctx->device->vendor_id == VK_VENDOR_ID_NVIDIA && ctx->device->architecture != vk_device_architecture::NVIDIA_PRE_TURING) || ctx->device->vendor_id == VK_VENDOR_ID_INTEL)) {
    ...
    case GGML_OP_SET_ROWS:
        {
            ...
            if (dst->type == GGML_TYPE_TURBO2_0 || dst->type == GGML_TYPE_TURBO3_0 || dst->type == GGML_TYPE_TURBO4_0) {
                ...
                // One 32-thread workgroup processes one TQ4_1S block.
                ne = ne / 32;
...
static void ggml_vk_turbo_wht(ggml_backend_vk_context * ctx, vk_context& subctx, const ggml_tensor * src0, ggml_tensor * dst) {
    const int direction  = dst->op_params[0];
    const int group_size = dst->op_params[1];
        ...
        case GGML_OP_MUL_MAT_ID:
            {
                ...
                if (src0_type == GGML_TYPE_TQ4_1S && op->op == GGML_OP_MUL_MAT_ID) {
                    return false;
                }
        ...
        case GGML_OP_FLASH_ATTN_EXT:
            {
                ...
                const bool turbo_kv = op->src[1]->type == GGML_TYPE_TURBO2_0 || op->src[1]->type == GGML_TYPE_TURBO3_0 || op->src[1]->type == GGML_TYPE_TURBO4_0 ||
                                      op->src[2]->type == GGML_TYPE_TURBO2_0 || op->src[2]->type == GGML_TYPE_TURBO3_0 || op->src[2]->type == GGML_TYPE_TURBO4_0;
                if ((turbo_kv || !coopmat2) && !(device->subgroup_shuffle && device->subgroup_vote)) {
        ...
        case GGML_OP_SET_ROWS:
            {
                ...
                const bool is_turbo = op->type == GGML_TYPE_TURBO2_0 ||
                                      op->type == GGML_TYPE_TURBO3_0 ||
                                      op->type == GGML_TYPE_TURBO4_0;
                if (is_turbo && op->src[0]->ne[0] % 128 != 0) {
                    return false;
                }
                const bool tq4_1s_subgroup_32 =
                    device->subgroup_size == 32 ||
                    (device->subgroup_size_control && device->subgroup_min_size <= 32 && device->subgroup_max_size >= 32);
                if (op->type == GGML_TYPE_TQ4_1S &&
                    (op->src[0]->ne[0] % 32 != 0 || !device->subgroup_arithmetic || !device->subgroup_shuffle || !tq4_1s_subgroup_32)) {
                    return false;
                }
                switch (op->type) {
                    case GGML_TYPE_F32:
