...
void ggml_cuda_flash_attn_ext(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    // Use the fused raw-KV MMA path for small TurboQuant decode batches.
    {
        const ggml_tensor * Q = dst->src[0];
        const ggml_tensor * K = dst->src[1];
        const ggml_tensor * V = dst->src[2];
        const int cc = ggml_cuda_info().devices[ggml_cuda_get_device()].cc;
        // Turbo2 remains on VEC because its fused path is not numerically equivalent.
        const bool turbo_matched = K->type == V->type &&
            (K->type == GGML_TYPE_TURBO4_0 || K->type == GGML_TYPE_TURBO3_0);
